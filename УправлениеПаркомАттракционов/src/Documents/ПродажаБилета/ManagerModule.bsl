...
ПараметрыОбласти.Вставить("Дата", ?(Выборка.Дата = Неопределено, "", Формат(Выборка.Дата, "ДЛФ=D;")));
ПараметрыОбласти.Вставить("Номер", УдалитьЛидирующиеНули(?(Выборка.Номер = Неопределено, "", Выборка.Номер)));
